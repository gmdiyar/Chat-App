...
def onStart():
    ...
    startButton.config(text="Server is running.", state='disabled')

startButton = ttk.Button(main_frame)
startButton.pack()
    
if ServerInstance.checkConnection():
    startButton.config(text="Server is running.", state='disabled')
else:
    startButton.config(text="Start Server", command=onStart, state='normal')
