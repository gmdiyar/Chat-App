...
class Server:
    ...
    def clientHandler(self, client, addr):
        try:
            loginData = client.recv(1024).decode(format)
            username = usernameData.decode('utf-8')
            ...
            while True:
                ...
                self.relayMessage(message, sender=client)
    ...
    def relayMessage(self, message, sender):
        for client in self.clients:
            if client != sender:
                try:
                    formattedMessage = f'{self.username}: {message}'
                    client.send(formattedMessage.encode('utf-8'))
                except Exception as e:
